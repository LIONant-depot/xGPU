...
namespace e10
{
    struct virtual_tree_tab : e10::asset_browser_tab_base
    {
        ...
        void AddResourcePopUp()
        {
            for (auto& E : m_AssetMgr.m_AssetPluginsDB.m_lPlugins)
            {
                if (ImGui::MenuItem(E.m_TypeName.c_str()))
                {
                    ...
                    auto LastGeneratedAsset = m_AssetMgr.NewAsset(E.m_TypeGUID, LibGUID, m_ParentGUID);

                    if (E.m_TypeGUID == e10::folder_type_guid_v)
                    {
                        m_AssetMgr.m_mLibraryDB.FindAsReadOnly(LibGUID, [&](const e10::library_db& Lib)
                            {
                                Lib.m_InfoByTypeDataBase.FindAsReadOnly(LastGeneratedAsset.m_Type, [&](const std::unique_ptr<e10::library_db::info_db>& InfoDB)
                                    {
                                        InfoDB->m_InfoDataBase.FindAsWrite(LastGeneratedAsset.m_Instance, [&](e10::library_db::info_node& Node)
                                            {
                                                Node.m_Info.m_Name = std::format("New Folder - {:X}", LastGeneratedAsset.m_Instance.m_Value);
                                            });
                                    });
                            });
                    }
                    ...
                    {
                        m_Browser.setSelection({}, LastGeneratedAsset);
                    }
                }
            }
        }
        ...
        void LeftPanel() override
        {
            ...
            {
                auto RootNodeGUID = xresource::full_guid{ m_AssetMgr.m_ProjectGUID.m_Instance, e10::folder_type_guid_v };
                ...
                , [&](const std::unique_ptr<e10::library_db::info_db>& Entry)
                {
                    std::function<void(const xresource::full_guid&, const xresource::full_guid&)> DisplayNodes = [&](const xresource::full_guid& GUID, const xresource::full_guid& ParentGUID)
                    {
                        ...
                        {
                            std::string_view Name = GUID.m_Instance == m_AssetMgr.m_ProjectGUID.m_Instance ? Path.c_str() : InfoEntry.m_Info.m_Name.c_str();
                                ...
                                if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload("DESCRIPTOR_GUID"))
                                {
                                    ...
                                    if (payload_n.m_bSelection)
                                    {
                                        ...
                                        if (bValidationFailed == false)
                                        {
                                            for (auto& S : m_SelectedItems)
                                            {
                                                if (auto Err = m_AssetMgr.MoveDescriptor(m_SelectedLibrary, S, payload_n.m_Parent, GUID); Err)
                                                {
                                                    // Handle the error
                                                    int a = 0;
                                                }
                                    ...
                                    else if (payload_n.m_Parent != GUID)
                                    {
                                        ...
                                        if (auto Err = m_AssetMgr.MoveDescriptor(L.first, payload_n.m_Source, payload_n.m_Parent, GUID); Err)
                                        {
                                            // Handle the error
                                            int a = 0;
                                        }
                            ...
                            else if (GUID.m_Instance != m_AssetMgr.m_ProjectGUID.m_Instance)
                            {
                ...
                {
                    std::string TypeView;
                    if (auto e = m_AssetMgr.m_AssetPluginsDB.m_mPluginsByTypeGUID.find(T->m_TypeGUID); e == m_AssetMgr.m_AssetPluginsDB.m_mPluginsByTypeGUID.end())
                    {
                        TypeView = std::format("{:X}", T->m_TypeGUID.m_Value);
                    }
                    else
                    {
                        TypeView = std::format("{} - {:X}", m_AssetMgr.m_AssetPluginsDB.m_lPlugins[e->second].m_TypeName, T->m_TypeGUID.m_Value);
                    }
                ...
                , [&](const std::unique_ptr<library_db::info_db>& Entry)
                {
                    ...
                    , [&](const e10::library_db::info_node& Node)
                    {
                        ...
                        for (auto& E : Node.m_lChildLinks)
                        {
                            ...
                            {
                                Temp.m_TypeNameView = m_AssetMgr.m_AssetPluginsDB.m_lPlugins[e->second].m_TypeName;
                            }
                        ...
                        for (auto& E : TempNodes)
                        {
                            ...
                            if (int PressType = WrappedButton2(E.m_ResourceGUID.m_Instance, std::format("\n\n\n\n\n\n{}", StringOne).c_str(), button_sz, Color, pIcon, held); PressType == 2)
                            {
                                ...
                                {
                                    m_Browser.setSelection(E.m_ResourceGUID, {});
                                }
                            }
                            else if (PressType == 1)
                            {
