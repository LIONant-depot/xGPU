                                ...
                                Entry->m_InfoDataBase.FindAsReadOnly(E.m_ResourceGUID.m_Instance, [&](const e10::library_db::info_node& InfoEntry)
                                {
                                    ...
                                    if (InfoEntry.m_bHasDependencies && InfoEntry.m_Dependencies.m_Assets.empty() == false )
                                    {
                                        if (ImGui::BeginMenu("  Assets"))
                                        {
                                            ...
                                            {
                                                auto pos = E.rfind('\\') + 1;
                                                if (ImGui::MenuItem(std::string_view(E.data() + pos, E.size()-pos).data()))
                                                {
                                                    ...
                                                    auto Str = std::format(L"explorer /select,{}\\{}", m_AssetMgr.m_ProjectPath, strXstr(E));
                                                    system(strXstr(Str).data());
                                                }
                                            }
                                    ...
                                    if (ImGui::MenuItem("  Log File"))
                                    {
                                        ...
                                        if (std::filesystem::exists(Str))
                                        {
                                            system(std::format("explorer /select,{}", strXstr(Str) ).c_str());
                                        }
                                    }
                                });
                            });
                        ...
                        Text = std::format(
                            ...
                            , [&]()->std::string
                            {
                                ...
                                if ( NodeInfo.m_Dependencies.m_Assets.empty() == false )
                                {
                                    ...
                                    {
                                        Assets = std::format( "{}\n        [{}] {}", Assets, static_cast<int>(&E - NodeInfo.m_Dependencies.m_Assets.data()), E );
                                    }
                                ...
                                if (NodeInfo.m_Dependencies.m_VirtualAssets.empty() == false)
                                {
                                    ...
                                    {
                                        Assets = std::format("{}\n        [{}] {}", Assets, static_cast<int>(&E - NodeInfo.m_Dependencies.m_VirtualAssets.data()), E);
                                    }
