    ...
    inline std::chrono::zoned_time<std::chrono::milliseconds> ConvertToStdTime(std::filesystem::file_time_type FileTime)
{
    ...
    std::variant<bool, std::string> has_content(std::wstring_view folder_path) noexcept
    {
        try
        {
            ...
            if (!std::filesystem::exists(path))
                return std::format("Error There is not such directory path: {}", strXstr(folder_path) );
            
            if (!std::filesystem::is_directory(path))
                return std::format("Error This given path is not a directory: {}", strXstr(folder_path));
    ...
    namespace compilation
    {
        ...
        struct historical_entry
        {
            ...
            using log = xcore::lock::object<communication, xcore::lock::spin>;
            entry                   m_Entry     = {};
            ...
            )
        {
            ...
            //
            {
                ...
                if (FindCaseInsensitive(CompilerFeedback.get().m_Log, {"[COMPILATION_SUCCESS]"}) != std::string::npos)
                {
                ...
                while (notify_info && FileMonitorChanges.m_bRunning.load())
                {
                    ...
                    else
                    {
                        ...
                        if (bSkipFile==false)
                        {
                            ...
                            {
                                auto TypeName = strXstr(std::wstring(std::wstring_view( FilePath.data(), pos )));
    ...
    struct process_info_job final : xcore::scheduler::job<0>
    {
        ...
        xcore::err LoadInfo( const std::wstring_view LibraryPath, const std::wstring_view Path, const std::wstring_view ResourcePath, const std::wstring_view RelativePath )
        {
            // Load the file
            ...
            {
                std::lock_guard Lk(m_lErrors);
                m_lErrors.get().push_back(std::format(L"Fail to find the info file[{}] with error[{}]", Path.data(), strXstr(Ec.message()) ));
                return xerr_failure_s("Fail to find Info file");
            }
            ...
            if (bDependencyExists)
            {
                ...
                if ( auto Err = Dependencies.Serialize(true, strXstr(DependencyPath), Context); Err )
                {
                    std::lock_guard Lk(m_lErrors);
            ...
            for (const auto& entry : std::filesystem::recursive_directory_iterator(Path))
            {
                ...
                if (std::filesystem::is_directory(entry.path()))
                {
                    if (entry.path().extension() == L".desc")
                    {
                        ...
                        Channel.SubmitJob( [this, pData = std::move(Data) ]
                        {
                            if (m_bCancel) return;
    ...
    struct library_mgr
    {
                            ...
                            bool bFoundChildType = Library->m_InfoByTypeDataBase.FindAsReadOnly(ItemToDelete.m_Type, [&](const std::unique_ptr<library_db::info_db>& ChildTypeInfoDB)
                            {
                                ...
                                bool bChildFound = ChildTypeInfoDB->m_InfoDataBase.FindForDelete(ItemToDelete.m_Instance, [&](library_db::info_node& ChildInfoNode)
                                {
                                    ...
                                    if (std::filesystem::exists(FolderPath))
                                    {
                                        ...
                                        if (ec)
                                        {
                                            Error = std::format("Error removing {} path with error code: {}", strXstr(FolderPath), ec.message());
                                            printf("Error: %s \n", Error.c_str());
                                            Error.clear();
                                        }
                                        else
        ...
        */
        ...
        xcore::err RenameDescriptor(const library::guid glibraryGUID, const xresource::full_guid& gDescriptor, std::string_view NewName, bool bNewChange = true )
        {
            xcore::err Error;
            ...
            if (foundLibrary == false)
            {
                Error = xerr_failure_s("The library was not found in the library database");
            }
        ...
        xcore::err MoveDescriptor(const library::guid& libraryGUID, const xresource::full_guid& sourceDescriptor, const xresource::full_guid& sourceParent, const xresource::full_guid& Target, bool bNewChange = true)
        {
            xcore::err Error;
 ...
 //           assert(sourceParent != Target);
        ...
        xresource::full_guid NewAsset(const library::guid LibraryGUID, xresource::full_guid ResourceGUID, const xresource::full_guid& ParentGUID, const std::string_view Name = {} )
        {
            ...
            {
                ResourceGUID.m_Instance = xresource::instance_guid::GenerateGUID();
            }
            ...
            bool bFindLib = m_mLibraryDB.FindAsReadOnly(LibraryGUID, [&](const std::unique_ptr<library_db>& Library )
            {
                ...
                xproperty::settings::context Context;
                if ( auto Err = Info.Serialize( false, strXstr(FinalPath).c_str(), Context); Err)
                {
                    printf("Fail to serialize the info file [%s] with error [%s]", strXstr(FinalPath).c_str(), Err.getCode().m_pString);
                    exit(1);
                }
        ...
        xcore::err OpenProject( std::wstring_view ProjectPath )
        {
            ...
            if (m_mLibraryDB.size() == 1 )
            {
                ...
                auto RootName = strXstr(m_ProjectPath);
                RootName = RootName.substr(RootName.rfind('\\') + 1);
                RootName = RootName.substr(0, RootName.rfind('.'));
                ...
                m_mLibraryDB.FindAsReadOnly(Library.m_GUID, [&](const std::unique_ptr<library_db>& ProjectDB)
                {
                    ProjectDB->m_InfoByTypeDataBase.FindAsReadOnly(RootGUID.m_Type, [&](const std::unique_ptr<library_db::info_db>& Info)
                    {
                        bHasRootFolder = Info->m_InfoDataBase.FindAsWrite(RootGUID.m_Instance, [&](library_db::info_node& Node )
                        {
                            ...
                            Node.m_Info.m_Name = RootName;
                            bHasRootFolder     = true;
                        });
                    });
                });
                if (bHasRootFolder == false) NewAsset(m_ProjectGUID, RootGUID, { 0,0 }, RootName);
                ...
                for (auto& P : m_AssetPluginsDB.m_lPlugins)
                {
                    ...
                    , [&](const std::unique_ptr<library_db::info_db>& Entry)
                    {
                        for ( auto& I : Entry->m_InfoDataBase )
                        {
                            ...
                            {
                                auto FileName = strXstr(I.second.m_Path);
                                if ( auto Err = I.second.m_Info.Serialize(false, FileName.c_str(), Context); Err )
                                {
                                    printf("Fail to save info file [%s] with error[%s]", FileName.c_str(), Err.getCode().m_pString);
                                }
                                else
                                {
        ...
        static void CompilingThreatWorker( library_mgr& LibMgr )
        {
            ...
            while( Compilation.MasterCheckExitOrWait() )
            {
                ...
                int          MaxHistoricalEntries   = {};
                {
                    ...
                    {
                        if ( pPlugin->m_DebugCompiler.empty() == false ) CompilerPath = std::format(L"{}\\{}", LibMgr.m_ProjectPath, strXstr(pPlugin->m_DebugCompiler) );
                        else
                        {
                            if (pPlugin->m_ReleaseCompiler.empty() == false) 
                            {
                                ...
                                CompilerPath = std::format(L"{}\\{}", LibMgr.m_ProjectPath, strXstr(pPlugin->m_ReleaseCompiler));
                            }
                            else
                            {
                    ...
                    else
                    {
                        if (pPlugin->m_DebugCompiler.empty() == false)
                        {
                            ...
                            CompilerPath = std::format(L"{}\\{}", LibMgr.m_ProjectPath, strXstr(pPlugin->m_DebugCompiler));
                        }
                        else
                        {
                            printf("ERROR: Both the release compiler and debug compiler has not paths\n");
                ...
                //
                {
                    ...
                    {
                        xcore::lock::scope lock( *NewEntry.m_Log );
                        Result = NewEntry.m_Log->get().m_Result;
                        if (Result == compilation::historical_entry::result::FAILURE ) NewEntry.m_Log->get().m_Log = strXstr(CommandLine) + "\n==============================\n" + NewEntry.m_Log->get().m_Log;
                    }
                                        ...
                                        else
                                        {
                                            ...
                                            for (auto& E : Node.m_Dependencies.m_Assets)
                                            {
                                                ...
                                                if (auto it = std::find(Dependencies.m_Assets.begin(), Dependencies.m_Assets.end(), E); it == Dependencies.m_Assets.end())
                                                {
                                                    ...
                                                    if( false == LibraryDB->m_AssetDataBase.FindAsWrite( strXstr(E), [&](library_db::asset& Asset)
                                                    {
                                                    ...
                                                    if (bRemoveEntry)
                                                    {
                                                        ...
                                                        LibraryDB->m_AssetDataBase.FindForDelete(strXstr(E), [&](library_db::asset&){} );
                                                    }
                                                }
                                                else
                                                {
