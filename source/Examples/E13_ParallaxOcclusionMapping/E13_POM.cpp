...
namespace e13
{
    ...
    {
        xmath::fmat4 m_L2C;
        xmath::fvec4 m_LocalSpaceLightPos;    // We store gamma in the w
        xmath::fvec4 m_AmbientLightColor;     // Color and Intensity
        xmath::fvec4 m_LightColor;            // Color and Intensity
        xmath::fvec4 m_LocalSpaceEyePos;
    };
};
...
int E13_Example()
{
    ...
    if (auto Err = xgpu::CreateInstance(Instance, { .m_bDebugMode = true, .m_bEnableRenderDoc = true, .m_pLogErrorFunc = e13::DebugMessage, .m_pLogWarning = e13::DebugMessage }); Err)
    ...
    xgpu::vertex_descriptor VertexDescriptor;
    {
        auto Attributes = std::array
        {
            ...
            {
                .m_Offset = offsetof(e13::draw_vert_btn, m_Position)
            ,   .m_Format = xgpu::vertex_descriptor::format::FLOAT_3D
            }
        ,   xgpu::vertex_descriptor::attribute
            {
                .m_Offset = offsetof(e13::draw_vert_btn, m_TexCoord)
            ,   .m_Format = xgpu::vertex_descriptor::format::FLOAT_2D
            }
        ,   xgpu::vertex_descriptor::attribute
            {
                .m_Offset = offsetof(e13::draw_vert_btn, m_Tangent)
            ,   .m_Format = xgpu::vertex_descriptor::format::SINT8_4D_NORMALIZED
            }
        ,   xgpu::vertex_descriptor::attribute
            {
                .m_Offset = offsetof(e13::draw_vert_btn, m_Normal)
            ,   .m_Format = xgpu::vertex_descriptor::format::SINT8_4D_NORMALIZED
            }
        ,   xgpu::vertex_descriptor::attribute
            {
                .m_Offset = offsetof(e13::draw_vert_btn, m_Color)
            ,   .m_Format = xgpu::vertex_descriptor::format::UINT8_4D_NORMALIZED
            }
        };
        auto Setup = xgpu::vertex_descriptor::setup
        {
            .m_VertexSize = sizeof(e13::draw_vert_btn)
    ...
    xgpu::pipeline PipeLine;
    {
        ...
        ,   .m_Shaders           = Shaders
        ,   .m_PushConstantsSize = sizeof(e13::push_constants)
        ,   .m_Samplers          = Samplers
        };
    ...
    xgpu::pipeline_instance PipeLineInstance;
    {
        ...
        //
        {
            ...
            // Load file
            //
            if (auto Err = xbmp::tools::loader::LoadDSS(Bitmap, L"../../Assets/StoneWal01_1K/Stone Wall 01_1K_Normal - Compress BC5.dds"); Err)
            {
                e13::DebugMessage(Err.getMessage());
            ...
            {
                e13::DebugMessage(xgpu::getErrorMsg(Err));
        ...
        //
        {
            ...
            // Load file
            //
            if (auto Err = xbmp::tools::loader::LoadDSS(Bitmap, L"../../Assets/StoneWal01_1K/Stone Wall 01_1K_Diffuse.dds"); Err)
            {
                e13::DebugMessage(Err.getMessage());
                std::exit(Err.getStateUID());
            }
        ...
        //
        {
            ...
            // Load file
            //
            if (auto Err = xbmp::tools::loader::LoadDSS(Bitmap, L"../../Assets/StoneWal01_1K/Stone Wall 01_1K_Ambient Occlusion.dds"); Err)
            {
                e13::DebugMessage(Err.getMessage());
                std::exit(Err.getStateUID());
            }
        ...
        //
        {
            ...
            // Load file
            //
            if (auto Err = xbmp::tools::loader::LoadDSS(Bitmap, L"../../Assets/StoneWal01_1K/Stone Wall 01_1K_Glossiness.dds"); Err)
            {
                e13::DebugMessage(Err.getMessage());
                std::exit(Err.getStateUID());
            }
        ...
        //
        {
            ...
            // Load file
            //
            if (auto Err = xbmp::tools::loader::LoadDSS(Bitmap, L"../../Assets/StoneWal01_1K/Stone Wall 01_1K_Roughness.dds"); Err)
            {
                e13::DebugMessage(Err.getMessage());
                std::exit(Err.getStateUID());
            }
        ...
        //
        {
            ...
            // Load file
            //
            if (auto Err = xbmp::tools::loader::LoadDSS(Bitmap, L"../../Assets/StoneWal01_1K/Stone Wall 01_1K_Height.dds"); Err)
            {
                e13::DebugMessage(Err.getMessage());
                std::exit(Err.getStateUID());
            }
    ...
    // Create mesh
    //
    auto Mesh = //xprim_geom::uvsphere::Generate( 30, 30, 2, 1 ); xmath::fvec2 UVScale{4,4};
                //xprim_geom::capsule::Generate( 30,30,1,4); xmath::fvec2 UVScale{3,3};
                 xprim_geom::cube::Generate( 4, 4, 4, 4, xprim_geom::float3{1,1,1} ); xmath::fvec2 UVScale{1,1};

    xgpu::buffer VertexBuffer;
    {
        if (auto Err = Device.Create(VertexBuffer, { .m_Type = xgpu::buffer::type::VERTEX, .m_EntryByteSize = sizeof(e13::draw_vert_btn), .m_EntryCount = static_cast<int>(Mesh.m_Vertices.size()) }); Err)
            return xgpu::getErrorInt(Err);

        (void)VertexBuffer.MemoryMap(0, static_cast<int>(Mesh.m_Vertices.size()), [&](void* pData)
            {
                auto pVertex = static_cast<e13::draw_vert_btn*>(pData);
...
\
    ...
    while (Instance.ProcessInputEvents())
    {
        ...
        static xmath::radian R{ 0 };
        R += xmath::radian{ 0.001f };
