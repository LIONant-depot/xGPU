...
struct e09::runtime_geom
{
    auto getVertexVectorFormat( xgeom::stream_info& StreamInfo ) const noexcept
    {
        ...
        static_assert((int)xgpu::vertex_descriptor::format::SINT_RGB10A2_4D_NORMALIZED  == (int)xgeom::stream_info::format::SINT_RGB10A2_4D_NORMALIZED);
        static_assert((int)xgpu::vertex_descriptor::format::UINT_RGB10A2_4D_NORMALIZED  == (int)xgeom::stream_info::format::UINT_RGB10A2_4D_NORMALIZED);
    ...
    bool Load( xgpu::device& Device, std::wstring_view FileName ) noexcept
    {
        xserializer::stream Steam;

        if (auto Err = Steam.Load(FileName, m_pGeom); Err)
        {
            e09::DebugMessage(Err.getMessage());
            return true;
        }
...
struct e09::runtime_texture
{
    ...
    {
        if (auto Err = xgpu::tools::bitmap::Create(m_Texture, Device, xbitmap::getDefaultBitmap()); Err)
            return Err;
...
struct e09::runtime_material_instance
{
    ...
    void Activate(xgpu::cmd_buffer& CmdBuffer, const xmath::fmat4& L2C)
    {
...
struct e09::runtime_geom_instance
{
    ...
    void Render(xgpu::cmd_buffer& CmdBuffer, const xmath::fmat4& L2C)
    {
...
int E09_Example()
{
    ...
    if( auto Err = RuntimeGeom.Load( Device, std::wstring_view{ L"../../bin/spider.xgeom"} ); Err )
        return -1;
  ...
  //  raw3d_inspector Inspector("Property", Device);
    ...
    { xmath::radian{ -0.230000168f }
    , xmath::radian{ -1.40999949f  }
    , xmath::radian{ 0.0f }
    };
    float          Distance = 122;
