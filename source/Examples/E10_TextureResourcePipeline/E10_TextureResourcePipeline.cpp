...
int E10_Example()
{
    ...
    MaterialMgr.CreateMaterialInstance(Device, BackgroundMaterialInstance, xbitmap::getDefaultBitmap());
    ...
    {
        BitmapInspector.Load( SelectedDescriptor.m_ResourcePath, true);
    ...
    while (Instance.ProcessInputEvents())
    {
        ...
        //
        {
            ...
            //
            {
                ...
                PushContants.m_Translation.setup(0);
                PushContants.m_UVScale = { 120.0f,120.0f };

                PushContants.m_ToGamma   = 2.2f;

                PushContants.m_ColorMask = xmath::fvec4(1);
                PushContants.m_Mode      = xmath::fvec4(0,0,1,1);
                PushContants.m_TintColor = xmath::fvec4(DrawOptions.m_BackgroundIntensity
                                                        , DrawOptions.m_BackgroundIntensity
                                                        , DrawOptions.m_BackgroundIntensity
                                                        , 1);
                PushContants.m_MipLevel = 0;
                PushContants.m_NormalModes.setup(0);
            ...
            if( SelectedDescriptor.m_InfoGUID.empty() == false && UserMaterialInstance.m_TextureRef.isValid() )
            {
                ...
                case draw_options::channels_mode::COLOR_ALPHA:
                    PushContants.m_ColorMask = xmath::fvec4(1);
                    PushContants.m_Mode = xmath::fvec4(1, 0, 0, MipMode);
                    break;
                case draw_options::channels_mode::NO_ALPHA:
                    PushContants.m_ColorMask = xmath::fvec4(1);
                    PushContants.m_Mode = xmath::fvec4(0, 0, 1, MipMode);
                    break;
                case draw_options::channels_mode::A_ONLY:
                    PushContants.m_ColorMask = xmath::fvec4(0, 0, 0, 1);
                    PushContants.m_Mode = xmath::fvec4(0, 1, 0, MipMode);
                    break;
                case draw_options::channels_mode::R_ONLY:
                    PushContants.m_ColorMask = xmath::fvec4(1, 0, 0, 0);
                    PushContants.m_Mode = xmath::fvec4(0, 1, 0, MipMode);
                    break;
                case draw_options::channels_mode::G_ONLY:
                    PushContants.m_ColorMask = xmath::fvec4(0, 1, 0, 0);
                    PushContants.m_Mode = xmath::fvec4(0, 1, 0, MipMode);
                    break;
                case draw_options::channels_mode::B_ONLY:
                    PushContants.m_ColorMask = xmath::fvec4(0, 0, 1, 0);
                    PushContants.m_Mode = xmath::fvec4(0, 1, 0, MipMode);
                    break;
                }

                if (BitmapInspector.m_pBitmap->getFormat() == xbitmap::format::BC3_81Y0X_NORMAL
                    && DrawOptions.m_DisplayInGammaMode != draw_options::display_gamma_mode::RAW_DATA_INFILE)
                {
                    PushContants.m_NormalModes = xmath::fvec4(1, 0, 0, 0);
                }
                else if (BitmapInspector.m_pBitmap->getFormat() == xbitmap::format::BC5_8YX_NORMAL
                    && DrawOptions.m_DisplayInGammaMode != draw_options::display_gamma_mode::RAW_DATA_INFILE)
                {
                    PushContants.m_NormalModes = xmath::fvec4(0, 1, 0, 0);
                }
                else
                {
                    PushContants.m_NormalModes = xmath::fvec4(0, 0, 0, 0);
                }

                PushContants.m_MipLevel = static_cast<float>(std::max(0, DrawOptions.m_ChooseMipLevel));

                PushContants.m_TintColor = xmath::fvec4(1);
                ...
                case draw_options::display_gamma_mode::RAW_DATA_INFILE:
                    if (BitmapInspector.m_pBitmap->getColorSpace() == xbitmap::color_space::SRGB)
                    {
                        PushContants.m_ToGamma = 2.2f;
                    }
                    else
