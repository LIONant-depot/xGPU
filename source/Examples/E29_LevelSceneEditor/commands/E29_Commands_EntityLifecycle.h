...
namespace e29::commands
{
    ...
    struct delete_entity_cmd : xundo::command_base
    {
        ...
        const char* getCommandHelp() const noexcept override
        {
            return "Deletes an entity and its whole child subtree (undoable - restores hierarchy, references, and prefab overrides on Undo). Usage: DeleteEntity -Scene hexguid -Id id";
        }
        void RegisterArguments() noexcept override
        {
            m_hScene = m_Parser.addOption("Scene", "Scene guid, 16 hex digits", true, 1);
            m_hId    = m_Parser.addOption("Id",    "Root entity permanent_id",  true, 1);
        }
