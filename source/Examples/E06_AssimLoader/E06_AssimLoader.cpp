...
struct static_geom
{
    ...
    bool Initialize( xgpu::device& Device, import3d::geom& Geom )
    {
        ...
        (void)m_VertexBuffer.MemoryMap(0, static_cast<int>(nVertices), [&](void* pData)
        {
            ...
            for (auto& Vert     : Submesh.m_Vertices)
            {
                ...
                pVertex->m_Color = xcolori(0xffffffff);
                pVertex++;
            }
        });
        ...
        xgpu::pipeline PipeLine;
        {
            ...
            , .m_Shaders            = Shaders
            , .m_PushConstantsSize  = sizeof(xmath::fmat4)
            , .m_Samplers           = Samplers
            };
    ...
    void Render(xgpu::cmd_buffer& CmdBuffer, const xmath::fmat4& L2C, std::uint64_t MeshMask = ~0ull)
    {
...
int E06_Example()
{
    ...
    static_geom StaticGeom;
    {
        ...
        if (auto Err = Importer.Import( L"../../Assets/demon-skull-textured/source/Skull_textured.fbx", &StaticRawGeom, nullptr, nullptr ); Err)
        {
            assert(false);
            return -1;
        }
