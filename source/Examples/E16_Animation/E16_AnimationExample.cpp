...
int E16_Example()
{
    ...
    //
    std::vector<xmath::fmat4> FinalL2W(AnimCharacter.m_Skeleton.m_Bones.size());
    const float ScaleBones = [&]
    {
        ...
        for( auto i=0u; i<AnimCharacter.m_Skeleton.m_Bones.size(); ++i )
        {
            if (AnimCharacter.m_Skeleton.m_Bones[i].m_iParent != -1)
            {
                ...
                auto V = M1.ExtractPosition() - M2.ExtractPosition();
                M1 = M1.Inverse();
                TotalL +=  ( M1 * V ).Length();
            }
        }
