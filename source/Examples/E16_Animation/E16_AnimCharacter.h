...
namespace e16
{
    struct anim_player
    {
        ...
        void ComputeMatrices( std::span<xmath::fmat4> FinalL2W, const xmath::fmat4& L2W ) const noexcept
        {
            auto&       Anim      = m_AnimPackage.m_Animations[m_iCurAnim];
            ...
            for (int i = 0; i < Anim.m_BoneKeyFrames.size(); ++i )
            {
                ...
                if constexpr (true)
                {
                    FinalL2W[i] = xmath::transform3::fromBlend(KeyFrame.m_Transfoms[iFrameT0], KeyFrame.m_Transfoms[iFrameT1], FrameTime - iFrameT0);
                }
                else
                {
                    FinalL2W[i] = KeyFrame.m_Transfoms[iFrameT0];
                }
